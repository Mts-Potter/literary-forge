...
export async function POST(request: NextRequest) {
  try {
    ...
    let validatedData
    try {
      validatedData = sceneDescriptionSchema.parse(body)
    } catch (err) {
    ...
    // Call Gemini to generate scene description
...
Output ONLY the summary of the story content. Even if incomplete, describe what is present. No apologies or explanations.`
    ...
    const description = (await invokeGemini(prompt, {
      maxTokens: 500,
      temperature: 0.5
    })).trim()
    ...
    // Detect error/apologetic responses from the LLM (treated as failures)
    const errorPatterns = [
      /i apologize/i,
      /i cannot/i,
      /does not contain/i,
    ...
    if (isErrorResponse) {
      console.log('LLM returned error/apologetic message, using fallback excerpt')
